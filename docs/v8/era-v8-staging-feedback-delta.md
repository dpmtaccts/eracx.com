...
## 28. Hero motion — atmospheric drift replaces playbook-maze

**Supersedes items 17 and 18.** The playbook-maze flow spec from item 17 and the tuning adjustments from item 18 are retired. The hero motion layer is now atmospheric drift.

**Reference file:** `era-v8-hero-drift-reference.html` (replaces `era-v8-hero-maze-reference.html` as the canonical hero build spec)

**The change:**

Playbook-maze was specific to ERA's brand metaphor (plays, routes, turns) but was visually busy and required scroll-ambient attention. Atmospheric drift is quieter and better matches ERA's product story: "operates the signals before they ever contact your team" describes background infrastructure, not kinetic activity. The motion should support that.

**What the new hero motion does:**

Three large blurred radial-gradient fields drift slowly in the background using long-duration ease-in-out transforms (24s, 28s, 30s). Each field has different color temperature:

- Field A — Mist Blue wash, top-left, largest (54rem)
- Field B — Deep Steel wash, top-right, mid-size (48rem)
- Field C — Magenta whisper at very low opacity (3-8%), bottom-center, smallest (34rem) — provides "one note of warmth" without reading as a brand color

A subtle gradient wash sits on top of the drift fields to keep foreground content legible.

**Implementation:**

Pure CSS. Three `<div>` elements positioned absolute inside a `.drift-layer` container, each with a `radial-gradient` background and a `@keyframes` animation. No SVG, no JavaScript, no external libraries. Filter blur at 80px for each field. `prefers-reduced-motion: reduce` pauses all animations — the drift fields remain visible as static gradients, so the space still reads as layered and atmospheric.

**What's retired from items 17 and 18:**

- Seven invisible SVG routes with `<path>` and `<animateMotion>`
- Flowing comet-tail dot clusters (35 animated circles across five routes after the item 18 reduction)
- Route trace lines and waypoint markers
- The outlined PLAYBOOK watermark — the drift fields ARE the background ornament now, the watermark was redundant

**What's added:**

- Eyebrow chip with pulsing Deep Steel dot at the top of the hero content
- Fade-rise animation on the hero content elements (eyebrow → headline → subhead → CTAs → trusted-by, staggered 100ms each)
- Theme toggle chip in the nav right side (Light / Dark) using the glassmorphic surface pattern. This is a visible, intentional UI element now, not a hidden icon button.
- Primary CTA gets a soft Deep Steel drop shadow (`0 10px 30px rgba(53, 80, 95, 0.18)`) — matches the layered-panel aesthetic
- Secondary CTA uses the glassmorphic surface style (`rgba(232, 226, 216, 0.72)` on light, backdrop-blur)

**Keep from prior hero specs:**

- Hero copy unchanged: "The modern B2B playbook. Run for you." (emphasis on "Run for you" in Deep Steel)
- Subhead unchanged: "ERA operates the signals, scoring, and outreach that shape how buyers decide before they ever contact your team."
- CTAs unchanged: primary "Build your new playbook", secondary "Request an audit"
- Trusted-by strip unchanged: Netrush · POP · Miniac · Navalent · Seismic

**The "too polite" guard still applies:**

Atmospheric drift risks drifting toward pale-wash aesthetic. The guards from Part 1 section 1.2 hold:

- Keep headline weight at Instrument Sans 700 — do not thin it to match the softer background
- Keep contrast high — body text is Ink on Parchment (not Graphite on Parchment, even though Graphite on a softly-washed ground would technically work)
- Keep CTAs at full Deep Steel saturation — do not tint them with the background color
- The magenta drift field is intentionally 3-8% opacity — if it ever reads as "pink hero," reduce further

**Reference file cleanup:**

- `era-v8-hero-maze-reference.html` can stay in the repo as an archive or be removed. It is no longer the canonical hero spec.
- `era-v8-hero-drift-reference.html` becomes the new canonical reference. Engineering implements against this file.
- V8 prompt section 3.2 (Hero) gets one line added noting the motion change: "Hero motion is atmospheric drift — see `era-v8-hero-drift-reference.html`. The earlier playbook-maze spec in this section is void per delta item 28."

---

## 29. Dark-ground accent color rule (patches item 27)

**Patches item 27.** The global token replacement in item 27 did not give engineering explicit guidance for accent colors on dark-ground sections within the light-theme site. This caused Deep Steel accents to render on Ink backgrounds in sections like the Signal River, where the contrast ratio is approximately 2.2:1 — failing WCAG AA for both body and large text.

**The rule:**

On any dark-ground section (background at `--color-ink` `#121417` or equivalent dark), the primary accent shifts from Deep Steel `#35505F` to Mist Blue `#A9BAC4`.

Contrast ratios on Ink:
- Deep Steel `#35505F` — 2.2:1 (fails AA for all text sizes)
- Mineral Blue `#4E6E81` — 3.9:1 (passes AA for large text only, fails for body)
- Mist Blue `#A9BAC4` — 10.4:1 (passes AAA for all text sizes)
- Gold `#C89B3C` — 8.2:1 (passes AAA — already being used correctly)

**Sections affected:**

All sections that use Ink as background color. Based on the page composition as specced:

1. **Signal River** — headline emphasis, tile outcome text, tile hover borders, active-tile indicators all shift to Mist Blue
2. **/our-story "Who runs it" block** (per item 25) — credential lines, hover states, link accents shift to Mist Blue
3. **Close section** (if dark ground per v5 pattern) — form field focus borders, Send button outline (not fill — fill uses full Deep Steel which reads fine as a filled surface), email link underline all shift to Mist Blue
4. **Footer** — link hover states, ERA wordmark accent elements shift to Mist Blue
5. **Any AI Mirror section variant rendered on dark ground** (if that treatment is used) — source panel header, chat bubble borders, UNLIKELY SOURCE tag keeps Magenta (Magenta on Ink passes 6:1)

**CSS token addition:**

Add a dark-ground accent variant to the light-theme token set:

```css
:root, [data-theme="light"] {
  /* existing tokens */
  --color-accent: #35505F;              /* Deep Steel — for light-ground sections */
  --color-accent-on-dark: #A9BAC4;      /* Mist Blue — for dark-ground sections within light theme */
  --color-accent-on-dark-hover: #C5D0D7; /* Mist Blue lifted — hover state on dark ground */
}
```

Engineering applies `--color-accent-on-dark` to any component that renders inside a dark-ground section. The cleanest pattern is to scope it via a section-level class or data attribute:
...
This lets all child components inherit the correct accent without being rewritten individually.

**What does NOT change:**

- Filled surfaces at full Deep Steel (like a primary CTA button) on dark ground are fine — the fill reads against the ground, contrast happens at the text/button boundary rather than in-text. Button labels inside Deep Steel fills use Parchment `#F4F1EA` for text, which is already high contrast.
- Gold for Wallet score stays Gold on dark ground (already passes).
- Magenta surprise accent stays Magenta on dark ground (passes).
- The dark-theme token set (when user clicks the Dark theme toggle) is separate and already uses `#6B8EA2` as its lifted primary accent. This rule only applies to dark-ground sections embedded within the light-theme site.

**QA check:**

After this patch, engineering should audit every dark-ground section and verify no text element uses `--color-accent` directly. All should resolve to `--color-accent-on-dark` via the section-level override. Running a color-contrast check (browser dev tools or axe) on every dark-ground section should return no failures.

---

## 30. Top nav restructure — simpler, product-forward

**Supersedes prior nav structure in all previous items.** The existing nav mixes scroll-anchor links to homepage sections (System, Signal Map, AUX, AI Mirror, Halo) with standalone pages (Our Story, Timeline). This reads as an internal site map, not a buyer's path. Replace it.
...
Plus the `Request an audit` CTA on the right side. All nav items are standalone pages, not homepage scroll anchors.

**Rationale for each item:**

| Item | What it is | Why it's in the nav |
|------|------------|---------------------|
| Why Era | Positioning page — thesis, three gaps, category claim | First-time visitors need the argument before they evaluate the offer |
| GTM Design | The fixed-scope diagnostic/audit offer — entry point product | Most buyers start here. Named per the deliverable's existing name (not "Launchpad," not "Playbook") |
| Halo | Standalone LinkedIn builder product, from $999/mo | Real product with its own depth and pricing, needs a real page |
| Free Tools | Hub for Relationship Map (map.eracx.com), Signal Map (signals.eracx.com), and insights/blog content | "Free Tools" beats "AI Tools" — honest, direct, conversion-lever naming |
| Our Story | Manifesto + who-runs-it page per item 25 | Personal/founder content lives here |
| Contact | Form or contact page | Always last in nav |

**CTA on the right:** `Request an audit` — routes to the GTM Design page or triggers the contact form modal. Stays the same.

**What's removed from the old nav:**

- System, Signal Map, AUX, AI Mirror, Timeline — these were scroll-anchor links to homepage sections. They're no longer nav items. Homepage sections are still anchored internally (for deep-linking and in-page navigation), but they don't clutter the top nav.

**Homepage impact:**

This restructure implies a shorter, more focused homepage. The homepage becomes a synthesis page — hero, proof strip, "Meet the new growth playbook," loops, Signal River (dark-ground section demonstrating signal variety), AUX surface demo, AI Mirror demo, audit section, Halo preview, What to Expect, Clients, Engage tiers, leader-bench, close.

The homepage no longer needs to be an exhaustive product tour because Halo, GTM Design, and Free Tools each have their own page. The homepage demonstrates the system, the product pages sell the specific offers.

**Page structure engineering needs to build:**

1. **`/` (homepage)** — the existing v8 build with all 29 items applied
2. **`/why-era`** — new page, content TBD (positioning essay, three gaps framework, category claim). Can defer to round 3
3. **`/gtm-design`** — new page, content pulled from the attached PDF (pipeline map, ICP, campaign architecture, GTM blueprint doc, the three-week engagement timing, fit criteria). This page is ready to build from the PDF
4. **`/halo`** — new page, content pulled from existing LinkedIn engine / Halo specs (dashboard surface, three principles: Voice-tuned not ghostwritten / Plugged into Signal River / Operated not advised, pricing from $999/mo)
5. **`/free-tools`** — new page, index of three sub-offerings:
   - Relationship Map (links out to `map.eracx.com`)
   - Signal Map (links out to `signals.eracx.com`)
   - Insights/Blog (either lives on-site at `/insights` or links to a separate content surface)
6. **`/our-story`** — per item 25, already specced
7. **`/contact`** — form page. Can reuse the close section's form component.

**Priority for engineering:**

- Round 3 scope: nav update is mandatory (affects every page). `/gtm-design`, `/halo`, and `/our-story` build alongside. These are the three pages with locked content.
- Round 4 or later: `/why-era`, `/free-tools`, `/contact`. Content needs to be written before these ship.

**Mobile behavior:**

Hamburger menu below the 768px breakpoint. Menu drawer slides from the right with the six nav items stacked vertically, each at 18px with 24px vertical padding, separated by hairline dividers. The `Request an audit` CTA appears at the bottom of the drawer at full width.

**Nav-bar styling (unchanged from item 19):**

Adaptive per IntersectionObserver. Transparent over light-ground sections, 90% dark with backdrop blur over dark-ground sections. 250ms transition. Logo on the left, nav items centered, theme toggle + CTA on the right.

**Section-anchor behavior on homepage:**

For users who want to jump to specific homepage sections (AUX, loops, etc.), anchor links still work via direct URL (`/#aux`, `/#loops`, etc.). These don't appear in the top nav but remain accessible from footer links, in-page CTAs, and deep-linking.

---

## 31. Surgical magenta + sky blue lift

**Patches item 27.** Two token-level changes. Self-contained.

### 31a — Add one approved magenta moment

Midcentury Magenta `#C44A7A` earns one more place beyond the three already specced in item 27 (AI Mirror UNLIKELY SOURCE tags, audit "noise" emphasis, rare chart callouts):

1. **Hero headline emphasis.** The span on "Run for you" flips from Deep Steel to Midcentury Magenta. This is the single highest-attention moment on the site. One visible pop, front of page, earned.

**Logo is out of scope.** The ERA logo and diamond mark exist in their current form. Do not modify colors, slashes, or symbols. Use the existing logo assets from `/public/images/` as-is.

**5% rule audit:** With this addition, magenta now appears in four approved places. Total visual weight across a full homepage scroll is estimated under 4% (hero emphasis ~1-2%, AI Mirror tags ~1%, audit "noise" emphasis ~1%, rare chart callouts <1%). Well under the 5% ceiling. Engineering should QA against a rendered-page screenshot at scroll complete — if magenta ever visually exceeds 5% surface area, pull back on the chart callouts first (lowest narrative value), then the audit emphasis.

**Specifically not changing:**
- Primary CTAs remain Deep Steel (`Build your new playbook`, `Request an audit`, section CTAs like `Get your audit`, form Send buttons)
- AUX composite score remains Deep Steel
- FRVRD radar polygon remains Deep Steel
- Activity feed deltas (+10, +12, etc.) remain Deep Steel
- Signal River tile outcome text on dark ground remains Sky Blue (with single anomaly accent in Magenta per existing "rare chart callout" rule)
- Leader-bench headline emphasis ("growth") remains Deep Steel

### 31b — Sky Blue replaces Mist Blue

Token rename and lightening:

```css
/* BEFORE (item 27 original) */
--color-mist: #A9BAC4;   /* Mist Blue */

/* AFTER (item 31) */
--color-sky: #C4D5DE;    /* Sky Blue — lighter, slightly bluer, less gray */
```

**Engineering action:** global rename of `--color-mist` to `--color-sky` across all CSS tokens and reference files. Update the hex from `#A9BAC4` to `#C4D5DE`.

**Dark-ground accent token (from item 29) updates accordingly:**

```css
/* BEFORE */
--color-accent-on-dark: #A9BAC4;      /* Mist Blue */
--color-accent-on-dark-hover: #C5D0D7;

/* AFTER */
--color-accent-on-dark: #C4D5DE;      /* Sky Blue */
--color-accent-on-dark-hover: #D5DDE4;
```

**Contrast on Ink:** Sky Blue `#C4D5DE` on Ink `#121417` measures approximately 11.2:1 — passes WCAG AAA for all text sizes (unchanged from Mist Blue's 10.4:1, but slightly higher).

**Where Sky Blue shows:** Signal River headline accent ("we watch"), Signal River tile outcome text, any dark-ground section link hover states, /our-story "Who runs it" section credentials, footer link hover states.

**Where Sky Blue does not show:** Any light-ground section. On Parchment, Sky Blue fails contrast for anything but the largest display type. Keep it scoped to dark-ground sections via the `data-ground="dark"` section override pattern from item 29.

### Reference file updates

Engineering updates all reference files in a single token-replacement pass:

- `era-v8-hero-drift-reference.html` — hero headline span flips to Magenta, Mist tokens rename to Sky with new hex
- `era-v8-aux-reference.html` — no changes (AUX stays Deep Steel primary)
- `era-v8-clients-reference.html` — stars and quote mark stay Deep Steel
- `era-v8-loops.html` — no changes (loops stay Deep Steel per item 27)
- `era-v8-new-playbook.html` — no changes (emphasis stays Deep Steel)
- V8 prompt section 1.2 — rename Mist Blue token to Sky Blue, update hex, add hero headline and logo slash to magenta approved-uses list

---

## 32. Two additional magenta moments — section eyebrow dots

**Extends item 31.** Adds two magenta placements to create a recurring brand signal across the scroll. Signal River headline flip is deferred pending staging review of this smaller change.

### 32a — Loops section eyebrow dot

The eyebrow pill above the "Run loops, not campaigns" headline currently contains a 7×7px Deep Steel dot. Flip that dot to Midcentury Magenta `#C44A7A`. No copy change. No structural change. Only the dot color updates.

Component spec (matches hero eyebrow pattern from item 31):

```html
<div class="loops-eyebrow">
...
.eyebrow-dot {
  ...
  height: 7px;
  border-radius: 50%;
  background: var(--color-magenta);  /* was var(--color-accent) */
}
```

### 32b — AUX section eyebrow dot

Same pattern applied to the "Inside AUX" eyebrow above the "Build the relationship infrastructure. Measured daily." headline. If no eyebrow dot currently exists in the AUX section, add one matching the Loops treatment — 7×7px, Magenta fill, 10px gap before the label text.

The current "Inside AUX" label may render as a standalone mono label without a pill container. Engineering should wrap it in the same `.eyebrow` pill container used by the hero and Loops sections for consistency.

### Rationale

These two moments alone won't fix the "invisible magenta" problem, but they work with the hero headline magenta (item 31a) to establish a recurring pattern: every major section eyebrow carries a magenta dot. Readers scrolling the page see magenta appear rhythmically — hero eyebrow, loops eyebrow, AUX eyebrow — then at specific content moments (AI Mirror UNLIKELY SOURCE, audit "noise"). It reads as brand signal, not decoration.

### What's deferred

The Signal River headline "we watch" accent flip (from Sky Blue to Magenta) is deferred. The Signal River is already the most visually dense section on the site (24-tile grid, dual-color tier system, dark ground), and adding magenta to the headline risks pushing it from "dense but readable" into "busy." Ship items 32a and 32b first, review in staging, decide on the Signal River headline based on how the combined magenta rhythm reads in context.

### Full magenta inventory after items 31 and 32 land

Six approved magenta moments across the homepage scroll:

1. Hero "Run for you" headline accent (item 31a)
2. Loops section eyebrow dot (item 32a, new)
3. AUX section eyebrow dot (item 32b, new)
4. AI Mirror UNLIKELY SOURCE tags on Glassdoor and Reddit (item 27)
5. Audit section "noise" emphasis (item 27)
6. Rare chart callouts, max one per chart (item 27)

Distribution: three moments on the top half of the scroll (hero, loops, AUX eyebrows), three on the bottom half (AI Mirror, audit, charts). Surface area well under 5%. Coverage is visible without saturating.

### QA after staging rebuild

Engineering should scroll the full homepage once items 31 and 32 are deployed and confirm:

- Magenta appears at least once every two full-viewport scrolls
- No magenta moment exceeds its specified size (dots stay at 7×7px, headline accent stays on the single "Run for you" phrase)
- Magenta never appears on CTAs, AUX composite score, FRVRD radar, or activity deltas — these stay Deep Steel
