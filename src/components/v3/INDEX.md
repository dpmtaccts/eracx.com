...
- **FrvrdSection.tsx** — §02. Framework intro between Thesis and LoopHalo. Two viewport-tall frames: a hero with eyebrow + "Five signals. One score." headline + sub + giant italic serif "Compounding." word that slides up + scales on viewport entry; below it, five FrvrdDimensionCards in canonical FRVRD order (Frequency, Recency, Value, Responsiveness, Density) with staggered fade-in and clip-path sparkline reveals. FloatingDotsBackground sits behind both frames. Bottom transition link anchors to HowItWorks. Honors `prefers-reduced-motion: reduce` (everything snaps; dots go static).
  - **FloatingDotsBackground.tsx** — Canvas of ~100 (50 mobile) drifting dots in --mist / --mineral-blue / --hot. Resolves CSS variables once via getComputedStyle. requestAnimationFrame drift loop paused by IntersectionObserver when the section leaves the viewport. Static single render under reduced motion.
  - **FrvrdDimensionCard.tsx** — White surface card with mono accent label, dashed-stroke sigmoid sparkline, and two-sentence definition. Stagger delay applied via `--frvrd-card-delay` custom property; entry animation triggered by an `is-visible` class on the parent .frvrd-frame--cards.
