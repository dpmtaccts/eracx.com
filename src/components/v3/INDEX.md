...
- **FrvrdSection.tsx** — ▸02. Pinned 4×100vh framework section, phased into four moments timed to scrollProgress: Phase 0 (0.00–0.20) holds the big static statement "Five signals turn noise into pipeline."; Phase 1 (0.20–0.30) fades the statement and previews a faint pentagon "ghost" outline while the FRVRD dots ramp from bg-blended to highlighted; Phase 2 (0.30–0.80) walks through the five dimensions one per 0.10 of progress, with axis labels appearing as each dot arrives and a center label slot-machining inside the polygon; Phase 3 (0.80–1.00) fills the polygon, completes the cold→warm color shift, and lands the closing "Warmth. / Ready to close." card in magenta. Below 820px the pin disables.
  - **BuildingFrvrdRadar.tsx** — Canvas centerpiece. ~220 (80 mobile) background "noise" dots drift constantly. Five FRVRD dots blend with the noise until scrollProgress 0.20, then ramp size + opacity to highlighted between 0.20 and 0.30 (`HIGHLIGHT_START`/`HIGHLIGHT_END`). Each migrates to its pentagon vertex inside its own 0.10-wide window (now 0.30–0.80). Pentagon stroke shows a faint "ghost" outline at alpha 0.15 from Phase 1 onward; per-segment alpha brightens to 1 as both endpoints settle. FRVRD layer color shifts cold→warm. Background dots stay neutral. Honors `prefers-reduced-motion: reduce`.
