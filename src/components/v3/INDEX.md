...
- **HowItWorks.tsx** — §03. Default renders the three horizontal figures (InteractionSequence / InteractionChain / WarmthRadars). A dev-only `import.meta.env.DEV`-gated toggle in the section header swaps to the vertical scrollytelling alternative (HowItWorksVertical). Selection persists in localStorage under `era-v3-howitworks-layout`. Production builds always render horizontal regardless of stored preference.
  ...
  - **HowItWorksVertical.tsx** — Vertical scrollytelling alternative for HowItWorks. Five full-height InteractionStage sections in the left column, sticky FRVRD radar in the right column at desktop. Opt-in via the dev toggle.
  - **InteractionStage.tsx** — Single full-height stage section. Three-column at desktop (visual mock + chain content + radar slot). IntersectionObserver bubbles the stage index up to HowItWorksVertical.
  - **StickyFrvrdRadar.tsx** — Sticky pentagon radar that morphs between cumulative per-stage values via requestAnimationFrame interpolation (800ms ease-in-out). The dimensions that move in each stage briefly pulse. Warmth counter ticks up smoothly. Honors `prefers-reduced-motion: reduce`.
