...
const sapiens: Player = {
  slug: 'sapiens', name: 'Sapiens', domain: 'sapiens.com', logo: '/images/insuretech/sapiens.svg', section: '06',
  ...
  channels: [
    ...
    CH.proof({
      score: { label: 'Customers & proof', value: 60, max: 100, interpretation: 'Proof carries the platform-depth promise and lags the AI promise. The named accounts are Clay-returned and not yet verified, so this is a prior read pending reference and review capture.', dims: [{ name: 'Named references', weight: 25, points: 16 }, { name: 'Proof depth and outcomes', weight: 25, points: 14 }, { name: 'Proof-to-promise fit', weight: 25, points: 12 }, { name: 'Verified installed base', weight: 25, points: 18 }] },
    ...
    CH.sources({
      score: { label: 'Credible sources', value: 50, max: 100, interpretation: 'The lightest analyst footprint of the four, validated by comparison-article citations rather than a Magic Quadrant Leader position. Real but secondary. The independent Gartner, Forrester, Celent, and Datos check is still to run.', dims: [{ name: 'Top-tier analyst placement', weight: 30, points: 12 }, { name: 'Peer-review weight', weight: 25, points: 12 }, { name: 'Trade and industry presence', weight: 20, points: 14 }, { name: 'Independence and verification', weight: 25, points: 12 }] },
...
const guidewire: Player = {
  slug: 'guidewire', name: 'Guidewire', domain: 'guidewire.com', logo: '/images/insuretech/guidewire.svg', section: '07', band: 'parchment',
  ...
  channels: [
    ...
    CH.proof({
      score: { label: 'Customers & proof', value: 78, max: 100, interpretation: 'The deepest verified base in the set, 1,700-plus implementations and a 4.6 Peer Insights rating across 105 reviews, and the proof fits the promise, including the AI turn, more fully than any rival. Prior-view figures that independent capture would confirm.', dims: [{ name: 'Named references', weight: 25, points: 20 }, { name: 'Proof depth and outcomes', weight: 25, points: 18 }, { name: 'Proof-to-promise fit', weight: 25, points: 16 }, { name: 'Verified installed base', weight: 25, points: 24 }] },
    ...
    CH.sources({
      score: { label: 'Credible sources', value: 79, max: 100, interpretation: 'Category authority, six straight years a Gartner Magic Quadrant Leader with the deepest verified review base. Held on the prior brand report; the independent re-verification is still pending, which holds the independence dimension down.', dims: [{ name: 'Top-tier analyst placement', weight: 30, points: 27 }, { name: 'Peer-review weight', weight: 25, points: 22 }, { name: 'Trade and industry presence', weight: 20, points: 16 }, { name: 'Independence and verification', weight: 25, points: 14 }] },
...
const majesco: Player = {
  slug: 'majesco', name: 'Majesco', domain: 'majesco.com', logo: '/images/insuretech/majesco.svg', section: '08',
  ...
  channels: [
    ...
    CH.proof({
      score: { label: 'Customers & proof', value: 63, max: 100, interpretation: 'Real proof of scale, over $100B in processed premium and a 4.5 Peer Insights rating, but the AI-native claim runs ahead of shipped outcomes, and one Clay-named account is a broker, not a carrier. Prior read pending verification.', dims: [{ name: 'Named references', weight: 25, points: 15 }, { name: 'Proof depth and outcomes', weight: 25, points: 17 }, { name: 'Proof-to-promise fit', weight: 25, points: 14 }, { name: 'Verified installed base', weight: 25, points: 17 }] },
    ...
    CH.sources({
      score: { label: 'Credible sources', value: 70, max: 100, interpretation: 'Analyst authority stronger than the revenue band predicts, a six-time Gartner MQ Leader with a 4.5 Peer Insights rating on the P&C Intelligent Core, plus owned research. Prior view pending independent re-verification and confirmation of full multi-line coverage.', dims: [{ name: 'Top-tier analyst placement', weight: 30, points: 24 }, { name: 'Peer-review weight', weight: 25, points: 19 }, { name: 'Trade and industry presence', weight: 20, points: 14 }, { name: 'Independence and verification', weight: 25, points: 13 }] },
...
const duckcreek: Player = {
  slug: 'duckcreek', name: 'Duck Creek', domain: 'duckcreek.com', logo: '/images/insuretech/duckcreek.svg', section: '09', band: 'parchment',
  ...
  channels: [
    ...
    CH.proof({
      score: { label: 'Customers & proof', value: 64, max: 100, interpretation: 'A deep enterprise base, 370-plus carriers including 33 of the top 50 in North America, but the SaaS-segment review base is thin, 3.2 across 17, and the Send-acquired AI is unproven at scale. Prior read.', dims: [{ name: 'Named references', weight: 25, points: 17 }, { name: 'Proof depth and outcomes', weight: 25, points: 16 }, { name: 'Proof-to-promise fit', weight: 25, points: 14 }, { name: 'Verified installed base', weight: 25, points: 17 }] },
    ...
    CH.sources({
      score: { label: 'Credible sources', value: 63, max: 100, interpretation: 'Solid placement, a Gartner Leader in 2025 SaaS P&C North America plus Everest, Celent, and IDC, but a thin SaaS-segment review base, 3.2 across 17, and a derivative framing against Guidewire. Prior view pending the trade-body check.', dims: [{ name: 'Top-tier analyst placement', weight: 30, points: 21 }, { name: 'Peer-review weight', weight: 25, points: 14 }, { name: 'Trade and industry presence', weight: 20, points: 15 }, { name: 'Independence and verification', weight: 25, points: 13 }] },
