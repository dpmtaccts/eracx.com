...
import type { Reviewer, KeySlide } from "../lib/reviewers";
...
@media (max-width: 768px) {
  ...
  .rv-floating-mic { bottom: 16px !important; right: 16px !important; }
}
`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ width: 8, height: 8, borderRadius: "50%", background: "rgba(255,255,255,0.08)" }} />
        <span style={{ width: 8, height: 8, borderRadius: "50%", background: "rgba(255,255,255,0.08)" }} />
        <div style={{ flex: 1, marginLeft: 8, height: 20, borderRadius: 4, background: "rgba(255,255,255,0.04)" }} />
      </div>
...
function AudioRecorder({
  ...
  return (
    <div id="voice-feedback" style={{ textAlign: "center" }}>
      ...
      <p style={{ fontSize: 15, fontWeight: 300, opacity: 0.5, maxWidth: 420, margin: "0 auto 28px" }}>
        {introText || "Hit record and share your honest thoughts. No need to be polished."}
      </p>
...
function FaqAccordion({ accent }: { accent: string }) {
      ...
      {FAQ_ITEMS.map((item, i) => {
        ...
        return (
          ...
          >
            ...
            >
              {item.question}
              <span style={{ fontSize: 18, opacity: 0.4, flexShrink: 0, color: isOpen ? accent : "#F6F5F2", transition: "transform 0.2s ease, color 0.2s ease", transform: isOpen ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
            </button>
            {isOpen && (
              <p style={{ fontSize: 14, fontWeight: 300, lineHeight: 1.8, opacity: 0.6, paddingBottom: 20, margin: 0 }}>
                {item.answer}
              </p>
            )}
          </div>
...
// ══════════════════════════════════════════════════════════════
// CURATED CARD (for personalized layout)
// ══════════════════════════════════════════════════════════════
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "28px 32px", flex: 1 }}>
        ...
        {/* Title */}
        <h3 style={{ fontSize: 18, fontWeight: 600, marginBottom: 12, color: "#F6F5F2" }}>{title}</h3>

        {/* Prompt */}
        <p style={{ fontSize: 14, fontWeight: 300, fontStyle: "italic", lineHeight: 1.7, opacity: 0.6, marginBottom: 20, maxWidth: 560 }}>
          "{prompt}"
        </p>

        {/* Password (if present) */}
        {passwordContent && (
          <div style={{ marginBottom: 16 }}>{passwordContent}</div>
        )}
...
function FloatingMicButton({ accent }: { accent: string }) {
...
// ══════════════════════════════════════════════════════════════
// SHARED SHELL (header, accent stripe, footer)
// ══════════════════════════════════════════════════════════════
...
function PersonalizedLayout({
  ...
  const { firstName, greeting, accent, heroImage, expertise, specificAsk, keySlides } = reviewer;
  const sectionGap = 96;
  ...
  return (
    <PageShell name={reviewer.name} accent={accent}>
      <main className="rv-container">
          ...
          <div className="rv-grid-hero">
            <div>
              ...
              <h1 className="rv-hero-title" style={{ fontSize: 40, fontWeight: 300, lineHeight: 1.2, marginBottom: 20 }}>
                Hey {firstName}<span style={{ color: accent }}>.</span>
              </h1>
              <p style={{ fontSize: 17, fontWeight: 300, lineHeight: 1.7, opacity: 0.85, marginBottom: 20 }}>
                {greeting}
              </p>
              {specificAsk && (
                <p style={{ fontSize: 17, fontWeight: 400, lineHeight: 1.7, color: accent, marginBottom: 20 }}>
                  The one thing I'd love your take on: {specificAsk}
                </p>
              )}
              <p style={{ fontSize: 14, fontWeight: 300, lineHeight: 1.8, opacity: 0.6 }}>
                The walkthrough below is a 10-minute overview. Below that, I've pulled out the specific slides and materials I'd love your reaction to. Voice recorder is available anywhere on the page: just click the mic in the corner.
              </p>
            </div>
            {heroImage ? (
              <a href={heroImage.href} target="_blank" rel="noopener noreferrer" style={{ display: "block", textDecoration: "none" }}>
                <div style={{
                  ...
                  <div style={{
                    ...
                    <span style={{ flex: 1, textAlign: "center", fontSize: 11, color: "rgba(255,255,255,0.35)", fontWeight: 400 }}>
                      eracx.com
                    </span>
                    <span style={{ width: 40 }} />
                  </div>
                  <img src={heroImage.src} alt={heroImage.alt} style={{ width: "100%", height: "auto", display: "block" }} />
                </div>
              </a>
            ) : (
        ...
        {/* ── What I Need Your Eyes On ── */}
        <section style={{ marginBottom: sectionGap, animation: "fadeUp 0.8s ease 0.15s both" }}>
          <p style={{ ...kicker, marginBottom: 12 }}>WHAT I NEED YOUR EYES ON</p>
          <p style={{ fontSize: 14, fontWeight: 300, lineHeight: 1.8, opacity: 0.55, marginBottom: 40, maxWidth: 640 }}>
            Instead of asking you to read everything, here are the specific pieces where your {expertise} lens matters most. React to as many or as few as you have time for.
          </p>

          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {/* Reviewer-specific slide cards */}
            {keySlides!.map((slide: KeySlide) => (
              <CuratedCard
                key={slide.number}
                badge={`SLIDE ${slide.number}`}
                title={slide.label}
                prompt={slide.prompt}
                linkHref={`${PITCH_DECK_PUBLISHED_URL}#slide=id.p${slide.number}`}
                linkLabel="Open in deck"
                accent={accent}
                secondaryAction={{ label: "Record a reaction", onClick: scrollToRecorder }}
              />
            ))}

            {/* Fixed card: Operator Playbook */}
            <CuratedCard
              badge="PLAYBOOK"
              title="How we recruit, train, and run operators"
              prompt="The section on compensation and pod structure is where most of my questions come from investors. Does it hold up?"
              linkHref={OPERATOR_PLAYBOOK_URL}
              linkLabel="Open playbook"
              accent={accent}
              secondaryAction={{ label: "Record a reaction", onClick: scrollToRecorder }}
            />

            {/* Fixed card: Atlas / Aux Platform */}
            <CuratedCard
              badge="AUX PLATFORM"
              title="The operating system behind the loops"
              prompt="This is a demo environment. Poke around and tell me: does it feel like a product or a dashboard?"
              linkHref={ATLAS_URL}
              linkLabel="Open Aux"
              accent={accent}
              secondaryAction={{ label: "Record a reaction", onClick: scrollToRecorder }}
              passwordContent={
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ fontSize: 12, opacity: 0.5 }}>Password:</span>
            ...
            <CuratedCard
              badge="PODCAST"
              title="How we work internally (audio)"
              prompt="If you're in the car or on a walk, this is the audio version of the operating model."
              linkHref={PODCAST_URL}
              linkLabel="Listen"
              accent={accent}
            />
          </div>
          ...
          {/* See all slides link */}
          <p style={{ marginTop: 24, fontSize: 13, opacity: 0.4 }}>
            Want the full picture?{" "}
            <a
              href={PITCH_DECK_PUBLISHED_URL}
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: accent, textDecoration: "none" }}
            >
              Open the complete deck
            </a>
          </p>
        </section>
        ...
        <section style={{ marginBottom: sectionGap, animation: "fadeUp 0.8s ease 0.3s both" }}>
          <p style={{ ...kicker, marginBottom: 32 }}>FREQUENTLY ASKED</p>
          <FaqAccordion accent={accent} />
        </section>
        ...
        <section style={{ marginBottom: sectionGap }}>
          <AudioRecorder
            ...
            accent={accent}
            introText="You can also use the recorder here for longer thoughts. Everything comes straight to me: no login, no account, no one else hears it."
          />
        </section>

        {/* ── What Comes Next ── */}
        <section style={{ marginBottom: sectionGap }}>
          <p style={{ ...kicker, marginBottom: 20 }}>WHAT COMES NEXT</p>
          <p style={{ fontSize: 16, fontWeight: 300, lineHeight: 1.85, opacity: 0.65, maxWidth: 580 }}>
            After I hear from everyone, I'll follow up with a short conversation. Part of that will be about your network: specifically, whether you know B2B companies with 3 to 15 in sales, no dedicated outbound function, and $50K+ average deal sizes. If there are people who come to mind, I'll make the introduction easy and low-friction for both sides. No ask until the conversation.
          </p>
        </section>
      </main>

      {/* Floating mic button */}
      <FloatingMicButton accent={accent} />
    </PageShell>
  );
}

// ══════════════════════════════════════════════════════════════
// LEGACY LAYOUT (for reviewers without keySlides)
// ══════════════════════════════════════════════════════════════
...
function LegacyLayout({
  ...
  greeting: string;
  accent: string;
  heroImage?: Reviewer["heroImage"];
}) {
  const sectionGap = 96;

  return (
    <PageShell name={name} accent={accent}>
      <main className="rv-container">
        {/* ── Hero: Greeting + Loom ── */}
        <section style={{ marginBottom: sectionGap, animation: "fadeUp 0.8s ease both" }}>
          <div className="rv-grid-hero">
          ...
          <div className="rv-grid-tools">
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "24px 24px 28px" }}>
                ...
                <a href="https://signals.eracx.com" target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 13, color: accent, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "24px 24px 28px" }}>
                ...
                <a href="https://map.eracx.com" target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 13, color: accent, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)", overflow: "hidden" }}>
              ...
              <div style={{ padding: "24px 24px 28px" }}>
                ...
                <a href="https://eracx.com/gtm-design" target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 13, color: accent, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>
...
// ══════════════════════════════════════════════════════════════
// MAIN PAGE — routes to personalized or legacy
// ══════════════════════════════════════════════════════════════
...
export default function Review() {
  ...
  // Personalized layout for reviewers with curated keySlides
  if (reviewer?.keySlides) {
    return <PersonalizedLayout reviewer={reviewer} slug={slug!} />;
  }

  // Legacy layout for everyone else
